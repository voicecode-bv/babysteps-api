...
#[Description('Generate small square thumbnails for image posts and user avatars that do not have one yet.')]
        ...
        $query->chunkById($chunk, function ($posts) use ($media, $bar, &$generated, &$skipped) {
            foreach ($posts as $post) {
                $path = $media->generateImageThumbnailFromPath(
                    $post->media_url, $post->user_id, 'posts', size: MediaUploadService::THUMBNAIL_SIZE_SMALL,
                );
    ...
    private function backfillAvatars(MediaUploadService $media, int $chunk): void
    {
        ...
        $query->chunkById($chunk, function ($users) use ($media, $bar, &$generated, &$skipped) {
            foreach ($users as $user) {
                $path = $media->generateImageThumbnailFromPath(
                    $user->avatar, $user->id, 'avatars', size: MediaUploadService::THUMBNAIL_SIZE_SMALL,
                );
